...
const self = module.exports = {
  ...
  matchPulls: (recentReviews) => {
    const matched = [];
    const stuff = [].concat(prVitalInfo);

    for (let i in recentReviews) {
      const matchedPull = stuff.filter(obj => obj.api_url.match(recentReviews[i]['review_pr']));
      const state = recentReviews[i]['review_state'];

      matchedPull.review_state = state;
      console.log(matchedPull);
    }

    console.log(matched);
    return matched;
  }, 
  // mapStuff: (recentReviews) => {
  //   promiseArr = [];

  //   for (let i in recentReviews) {
  //     if (Object.values(recentReviews[i]).indexOf('COMMENTED') > -1) {

  //     } else {
  //       console.log('other shit', recentReviews[i]['review_pr']);
  //     }
  //   }
