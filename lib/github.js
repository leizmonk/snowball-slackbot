...
const self = module.exports = {
  ...
  approvedOrNot: (matchedPulls) => {
    ...
    const reviewReducer = (accumulator, prId) => {
      const approvedPulls = matchedPulls.filter((pull) => pull.pr_id === prId && pull.review_state === 'COMMENTED');
      const unApprovedPulls = matchedPulls.filter((pull) => pull.pr_id === prId && pull.review_state !== 'COMMENTED');
